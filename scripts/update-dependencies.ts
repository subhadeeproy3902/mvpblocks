...
function main() {
  ...
  const npmDepsMatch = autoRegisterContent.match(/const KNOWN_NPM_DEPENDENCIES = \[([\s\S]*?)\];/);
  if (!npmDepsMatch) {
  ...
  const registryDepsMatch = autoRegisterContent.match(/const REGISTRY_DEPENDENCIES: Record<string, string> = \{([\s\S]*?)\};/);
  ...
  const npmDeps = Object.keys({ ...packageJson.dependencies, ...packageJson.devDependencies });
