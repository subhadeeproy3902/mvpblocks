...
```css
.avatar-custom {
  @apply h-16 w-16 border-4 border-primary rounded-full;
}
```
...
```css
.tooltip-custom {
  @apply bg-gradient-to-r from-blue-600 to-purple-600 text-white px-6 py-3 rounded-lg;
}
```

### Gradient Effects
The tooltip includes two gradient lines:
- **Emerald gradient**: 20% width, via-emerald-500
- **Sky gradient**: 40% width, via-sky-500
