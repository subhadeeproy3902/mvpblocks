...
> **Note:** If you are using `ui` or `hooks`, make sure to add them in `registry-ui.ts` or `registry-hooks.ts` respectively. Main block path should be `@/components/mvpblocks/[category]/[block].tsx`.
...
- Make sure to list all registry dependencies in `registryDependencies`. A registry dependency is the path of the component in the registry eg. `https://mvpblocks.vercel.app/r/globe.json`.
